...
const Login = () => {
  ...
  const onSubmitHandler = async (event) => {
    event.preventDefault();
        ...
        console.log({ token, backendUrl, navigate, currentState });
    try {
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col items-center w-[90%] sm:max-w-96 m-auto mt-14 gap-4 text-gray-800'>
      ...
      {currentState === 'Log In' ? '' : <input onChange={(e) => setName(e.target.value)} value={name} className='w-full px-3 py-2 border border-gray-800 rounded-sm' type='text' placeholder='Name' required />}
      ...
      <button type="submit" className='bg-black text-white px-8 py-3 mt-4 cursor-pointer'>{currentState === 'Log In' ? 'Sign In' : 'Sign Up'}</button>
    </form>
  )
}
